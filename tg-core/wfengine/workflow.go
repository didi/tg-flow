...
/*func (w *WorkflowBranch) selectBranchKey(params map[string]interface{}) (string, error) {
	if !w.hasBranch() {
		return BranchKeyDefault, nil
	}

	branchMap := make(map[string]int)
	for _, actionId := range w.SortedBranch {
		idx, err := w.ActionMap[actionId].executeCond(params)
		if err != nil {
			return "", fmt.Errorf("execute cond error:%v", err)
		}
		branchMap[actionId] = idx
	}

	branchKey := w.getBranchKey(branchMap)
