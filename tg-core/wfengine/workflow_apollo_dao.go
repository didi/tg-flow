...
func LoadSceneModuleWorkflowFromApollo(namespace,  configName string) (map[int64]*SceneModule, map[int64]*Workflow, error) {
	apolloConfig, err := apollo.NewApolloConfig(namespace, configName)
	if err != nil || apolloConfig == nil {
		return nil, nil, fmt.Errorf("init apolloConfig fail, namespace=%v, configName=%v, err=%v", namespace, configName, err)
	}

	configParams := apolloConfig.GetConfigs()
	if configParams == nil || configParams[sceneKey] == "" {
		return nil, nil, fmt.Errorf("apollo GetConfigs fail, some keys not found namespace=%v, configName=%v, configParams=%v", namespace, configName, configParams)
	}

	sceneModuleString := ""
	for key, val := range configParams {
		if key == sceneKey {
			sceneModuleString = val
			break
		}
	}

	smMap, err := loadSceneModuleString(sceneModuleString)
	if err != nil {
		return nil, nil, err
	}
