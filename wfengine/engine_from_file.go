...
func NewWorkflowEngineFromFile(moduleObj ModuleObjBase, configPath string) (*WorkflowEngine, error) {
	...
	if err1 != nil {
		tlog.Handler.ErrorCount(context.TODO(), "GetLatestVersionFromRedis_err", fmt.Sprintf("configPath=%v, err=%v", configPath, err1))
	}
...
func LoadWorkflowFromFile(dirPath string, smMap map[int64]*SceneModule) (map[int64]*Workflow, error) {
	...
	for filePath, _ := range filePaths {
		...
		if wf == nil || err != nil {
			tlog.Handler.ErrorCount(context.TODO(), "loadWorkflowFromFile_err", fmt.Sprintf("wf:%v,err:%v", wf, err))
			continue
		}
