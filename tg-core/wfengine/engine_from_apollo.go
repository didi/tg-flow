...
import (
	...
	"github.com/didi/tg-flow/tg-core/wfengine/apollo"
	"strconv"
	"strings"
)
...
func NewWorkflowEngineFromApollo(moduleObj ModuleObjBase, namespace, configName string) (*WorkflowEngine, error) {
	smMap, wfMap, version, err := loadSceneModuleWorkflowFromApollo(namespace, configName)
	if err != nil {
		return nil, err
	}
...
func loadSceneModuleWorkflowFromApollo(namespace, configName string) (map[int64]*SceneModule, map[int64]*Workflow, string, error) {
	apolloConfig, err := apollo.NewApolloConfig(namespace, configName)
	if err != nil || apolloConfig == nil {
		return nil, nil, "", fmt.Errorf("NewApolloConfig error, namespace=%v, configName=%v, err=%v", namespace, configName, err)
	}

	configParams := apolloConfig.GetConfigs()
	if configParams == nil {
		return nil, nil, "", fmt.Errorf("GetConfigs error, configParam is nil, namespace=%v, configName=%v", namespace, configName)
	}

	if configParams[sceneKey] == "" {
		return nil, nil, "", fmt.Errorf("configParams error, sceneKey is empty, namespace=%v, configName=%v, sceneKey=%v", namespace, configName, sceneKey)
	}
...
func LoadWorkflowFromApollo(configParams map[string]string) (map[int64]*Workflow, error) {
	...
	for key, val := range configParams {
		...
		if wf == nil || err != nil {
			tlog.ErrorCount(context.TODO(), "loadWorkflowFromKV_err", fmt.Sprintf("wf:%v,err:%v", wf, err))
			continue
		}
